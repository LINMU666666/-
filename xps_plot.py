...
# --- Plotting style settings (Times New Roman, inward ticks, SCI journal standards) ---
...
def generate_schematic_xps():
    """ Generate schematic XPS peak-fitting plot for Mn-Fe(2:1)/AC catalyst """

    fig, axs = plt.subplots(2, 2, figsize=(10, 8), sharex=False, sharey=False)

    title_font = {'size': 14, 'weight': 'bold'}

    # (a) Mn 2p (2p3/2) - schematic core active sites
    x = np.linspace(638, 650, 200)
    # Schematic data generation (actual processing requires imported data)
    peak1 = 40.2 * np.exp(-((x - 641.0) / 1.5)**2)  # Mn2+
    peak2 = 52.1 * np.exp(-((x - 642.1) / 1.5)**2)  # Mn3+ (core)
    peak3 = 7.7 * np.exp(-((x - 643.3) / 1.5)**2)   # Mn4+
    background = np.linspace(50, 100, 200) # schematic linear background
    ...
    axs[0, 0].plot(x, peak2, 'b--', lw=1.2, label='Mn$^{3+}$ (52.1%)') # blue highlight
    ...
    # (b) Fe 2p (2p3/2) - schematic promoter and sacrificial sites
    ...
    axs[0, 1].plot(x, peak2, 'b--', lw=1.2, label='Fe$^{3+}$ (68.5%)') # blue highlight
    ...
    # (c) O 1s - schematic oxygen vacancy concentration (Oβ)
    x = np.linspace(526, 536, 200)
    peak1 = 42.0 * np.exp(-((x - 529.8) / 1.5)**2)  # Oα Lattice
    peak2 = 58.0 * np.exp(-((x - 531.6) / 1.5)**2)  # Oβ Adsorbed (core)
    ...
    axs[1, 0].plot(x, peak2, 'b--', lw=1.2, label='O$_{\\beta}$: Adsorbed (58.0%)') # blue highlight
    ...
    # (d) C 1s - schematic carbon carrier functional groups
    x = np.linspace(280, 292, 200)
    peak1 = 71.2 * np.exp(-((x - 284.8) / 1.2)**2)  # C-C (calibration)
    ...
    plt.tight_layout()
    plt.savefig('XPS_high_std_schematic.png', dpi=300) # save as high-resolution image
    print("High-standard SCI XPS peak-fitting schematic generated: XPS_high_std_schematic.png")
